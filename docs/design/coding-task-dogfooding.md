...
- 默认总预算为 500,000 tokens。
- 固定 Handoff 预算为 100,000 tokens，不允许调用者修改。
- 有限总预算必须大于 100,000；Work Goal 使用 `total - 100,000`，Handoff 使用 `min(total, Work 累计用量 + 100,000)` 作为同一 Session 的累计上限。
- `unlimited` 只让 Work Goal 不受有限预算约束；Work Goal blocked 后的 Handoff 以 Work 累计用量加 100,000 作为累计上限。
