...
4. **不可变兼容组合与历史边界**：固定 `tutar/codex-action@393ad456e354dc9da7be630c09be243cc1d212af` 和 Codex CLI `0.153.4`；操作说明列出 runner、认证、permission profile 与持久目录边界。历史 `github-development-ticket/v0.1.2` 和已发布 Release/CLI 内容不变，旧 app-server Controller 仅从当前 Candidate 源移除。
