# github-pr-review v0.1.2 Candidate
...
配置文件不提供 runner、认证方式、Action source/revision、Codex CLI version、输入输出 mapping、Profile implementation、Provider 或 capability 开关。缺字段、未知字段、未允许值、Profile 身份不符或静态 Profile 任一契约被修改时，router 输出 `configuration-handoff`、不启动 review，也不自动换 runner、认证、Action/Provider 或降低安全限制。修改 runner labels、认证方式、Action SHA、CLI version、mapping、Executor 或 Profile 后，该副本已退出 `github-pr-review/v0.1.2` Candidate 声明；官方升级必须发布新的 Definition 版本并重新验证。
...
```bash
node --test workflow-definitions/github-pr-review/v0.1.2/test/*.test.mjs
```
