...
下一 Repository Release `v0.2.1` 将发布使用 Runner-backed Codex Authentication（Runner 承载的 Codex 认证）的 `github-pr-review/v0.1.1` 修正版 Candidate，并保留 `v0.2.0` / `github-pr-review/v0.1.0` 的历史版本。Repository Release 与 Workflow Definition 各自版本化；Repository Release 版本不等于 Definition 版本，也不代表 Candidate 已经真实兼容或晋级 Stable。

## 定义状态

- [`github-pr-review/v0.1.1`](workflow-definitions/github-pr-review/v0.1.1/README.md)：默认演进方向，当前为 Candidate；使用专用 self-hosted runner 的既有 Codex 登录状态，已通过自动化 Candidate Publication Gate，但尚无 Stable Supported Profile。
...
将 [`github-pr-review/v0.1.1/files/.github/`](workflow-definitions/github-pr-review/v0.1.1/files/.github/) 整体复制到 Consumer Project 的 `.github/`，安装项目自己的 `code-review` Skill，并提供带 `[self-hosted, Linux, X64, codex]` labels 与既有 Codex 登录状态的专用 runner，再按 [Candidate 文档](workflow-definitions/github-pr-review/v0.1.1/README.md) 编辑唯一允许的 `pr-review-config.json`。该 Definition 不使用仓库 `OPENAI_API_KEY`，只做只读 PR Review，并由独立可信 publish job 创建或更新 Check Run。

这一路线当前是 Candidate，不应把本仓库的静态测试、Fake Action 或 Agent final text 当成真实 Consumer 兼容证据。Candidate Gate 详情见 [`CANDIDATE-GATE.md`](workflow-definitions/github-pr-review/v0.1.1/CANDIDATE-GATE.md)。
...
- [`github-pr-review` v0.1.1 Candidate](workflow-definitions/github-pr-review/v0.1.1/README.md)：计划由 Repository Release `v0.2.1` 发布的 Runner-backed Codex Authentication 修正版；没有 Stable Supported Profile。
...
Workflow Definition 是普通文件模板，不是 Reusable Workflow，也不是中央 Runtime。Definition 版本只表示实例最后参考的定义版本；当前仓库发布说明见 [`v0.2.1`](docs/releases/v0.2.1.md)，历史版本见 [`v0.2.0`](docs/releases/v0.2.0.md)。
