...
当前唯一交付源是 [`workflow/.github/`](workflow/.github/)，CLI 一次安装整套文件。当前只交付 [Development Task（研发实现任务）](workflow/README.md)，保留票据标签、人工入口、实现、自测、review、commit、push、Draft PR 与原有恢复行为。
...
已发布 Repository Release `v0.3.0` 与 CLI `0.3.0` 保留原有内容；本分支不创建新 Release，不冻结 Candidate，也不承诺新的 Stable Definition（稳定工作流定义）。本地回归、静态交付核验与真实 Consumer 运行是不同证据。
