...
- Runtime 日志：Action 默认 `safe`；Coding Workflow 显式使用 `detailed`，按 `[codex][work|handoff][event]` 前缀近实时投影固定 allowlist 的 App Server 事件
...
`detailed` 日志包含 Goal 更新、TUI 可见 reasoning summary、Agent message、命令与逐行输出、命令终态、MCP 调用/进度/结果及相关文件变更。Action 将 Runtime 内容视为不可信数据：每一行都带固定前缀，控制字符被转义，已知 Action secrets 被脱敏；认证/账户、raw JSON-RPC、隐藏 reasoning 与未知事件不输出。renderer 单个事件失败时继续 Goal，App Server 协议失败仍使 Action 失败。`safe` 只输出阶段、事件、状态、token、elapsed time 与退出码；`silent` 不投影这些事件。
