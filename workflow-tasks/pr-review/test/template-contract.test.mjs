...
test("the Profile declares the immutable mapping and every validation boundary", () => {
  assert.equal(profile.definition, "github-pr-review/current");
  assert.equal(profile.executor.id, "codex-compatible-executor");
  assert.equal(profile.executor.version, "current");
...
test("the candidate output schema requires exact identity and both review axes", () => {
  assert.deepEqual(schema.required, ["repository", "pullRequestNumber", "baseSha", "headSha", "standards", "spec", "handoff"]);
  assert.equal(schema.properties.standards.anyOf[0].$ref, "#/$defs/axis");
  assert.equal(schema.properties.spec.anyOf[0].$ref, "#/$defs/axis");
  assert.deepEqual(schema.$defs.axis.required, ["verdict", "findings"]);
  assert.equal("oneOf" in schema, false);
  assert.deepEqual(schema.properties.handoff.type, ["object", "null"]);
