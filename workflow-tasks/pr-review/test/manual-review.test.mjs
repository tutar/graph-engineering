...
test("the manual Workflow Instance runs a controlled Action and publishes its trusted Check", async () => {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (request.method === "GET" && request.url.startsWith("/repos/acme/widgets/commits/") && request.url.includes("/check-runs")) {
      ...
      response.end(JSON.stringify({
        check_runs: [
          { id: 99, name: "tests", head_sha: currentHead, external_id: null, conclusion: "success" },
  ...
  try {
    ...
    assert.match(captured.runtime.summary, /tutar\/codex-action/);
    ...
    assert.equal(pullRequestReads, 10, "preparation and publication reread PR facts; successful publication also rechecks after history lookup");
