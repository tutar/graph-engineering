...
这是主分支中持续演进的 PR Review Task（PR 审查任务）。将 `files/.github/` 整体复制到 Consumer Project（消费项目）后，Consumer 自行拥有和维护这些普通文件；运行时不访问 `loop-engineering`，也不调用远程 Reusable Workflow（可复用工作流）。
...
1. 将 [`files/.github/`](files/.github/) 复制到 Consumer Project 根目录的 `.github/`。
2. 在 Consumer Project 安装 `code-review` Skill 并保留项目自己的规范与规格来源。
3. 提供带 `[self-hosted, Linux, X64, codex]` labels、且 Actions 服务用户已有 Codex 登录状态的专用 runner。
4. 只在 `.github/loop-engineering/pr-review-config.json` 的公开配置边界内调整事件、Event Prompt、model/effort 和 Check 展示文本。
